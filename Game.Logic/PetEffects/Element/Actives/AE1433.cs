...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1433 : BasePetEffect
    {
        ...
        {
            AE1433 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1433) as AE1433;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                {
                    enemy.AddPetEffect(new CE1433(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
                //Console.WriteLine("Buff ID:{0}, Name:{1}, success!", ElementInfo.ID, ElementInfo.Name);
            }
        }        
    }
}
