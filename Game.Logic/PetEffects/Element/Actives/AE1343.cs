...
{
    public class AE1343 : BasePetEffect
    {
        ...
        public AE1343(int count, int probability, int type, int skillId, int delay, string elementID)
            : base(ePetEffectType.AE1343, elementID)
        {
            m_count = count;
            m_coldDown = count;
        ...
        {
            AE1343 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1343) as AE1343;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            ...
            {
                m_added = (int)(player.BaseDamage * 45 / 100);
                player.BaseDamage += m_added;
