...
{
    public class AE1346 : BasePetEffect
    {
        ...
        public AE1346(int count, int probability, int type, int skillId, int delay, string elementID)
            : base(ePetEffectType.AE1346, elementID)
        {
            m_count = count;
            m_coldDown = count;
        ...
        {
            AE1346 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1346) as AE1346;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            ...
            {
                m_added = (int)(player.BaseGuard * 30 / 100);
                player.BaseGuard -= m_added;
