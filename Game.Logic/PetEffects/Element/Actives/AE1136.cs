...
{
    public class AE1136 : BasePetEffect
    {
        ...
        public AE1136(int count, int probability, int type, int skillId, int delay, string elementID)
            : base(ePetEffectType.AE1136, elementID)
        {
            m_count = count;
            m_coldDown = count;
        ...
        {
            AE1136 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1136) as AE1136;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                player.AddPetEffect(new CE1136(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
