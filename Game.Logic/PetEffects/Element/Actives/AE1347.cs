...
{
    public class AE1347 : BasePetEffect
    {
        ...
        public AE1347(int count, int probability, int type, int skillId, int delay, string elementID)
            : base(ePetEffectType.AE1347, elementID)
        {
            m_count = count;
            m_coldDown = count;
        ...
        {
            AE1347 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1347) as AE1347;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            ...
            {
                m_added = (int)(player.BaseGuard * 25 / 100);
                player.BaseGuard -= m_added;
