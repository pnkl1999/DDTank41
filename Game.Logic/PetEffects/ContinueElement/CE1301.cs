...
namespace Game.Logic.PetEffects.ContinueElement
{
    public class CE1301 : BasePetEffect
    {
        ...
        {
            CE1301 effect = living.PetEffectList.GetOfType(ePetEffectType.CE1301) as CE1301;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            ...
            {
                PE1301 effect = enemy.PetEffectList.GetOfType(ePetEffectType.PE1301) as PE1301;
                if (effect != null)
                {
                    if(player.PetMP > 0)
                    {
