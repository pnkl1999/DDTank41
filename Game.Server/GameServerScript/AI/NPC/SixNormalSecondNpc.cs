    ...
    public class SixNormalSecondNpc : ABrain
    {
        ...
        private void MovePlace()
        {
            if(m_stepMove >= m_point.Count)
            {
                ...
                //Body.ChangeDirection(Game.FindlivingbyDir(Body), 0);
                Body.MoveTo(pAward.X, pAward.Y, "flyLR", 0, "happy", 8, null);
            ...
            {
                Body.MoveTo(p.X, p.Y, sAction, 0, 8, MovePlace);
            }
            else
            {
                m_countStepMove = m_stepMove + Body.Config.MaxStepMove;
                Body.MoveTo(p.X, p.Y, sAction, 0, 8);
            }
        }
